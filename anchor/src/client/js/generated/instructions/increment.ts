...
import { CERTFRONT_PROGRAM_ADDRESS } from '../programs';
...
export type IncrementInstruction<
  TProgram extends string = typeof CERTFRONT_PROGRAM_ADDRESS,
  TAccountCertfront extends string | IAccountMeta<string> = string,
  TRemainingAccounts extends readonly IAccountMeta<string>[] = [],
> = IInstruction<TProgram> &
  IInstructionWithData<Uint8Array> &
  IInstructionWithAccounts<
    [
      TAccountCertfront extends string
        ? WritableAccount<TAccountCertfront>
        : TAccountCertfront,
      ...TRemainingAccounts,
    ]
  >;
...
export type IncrementInput<TAccountCertfront extends string = string> = {
  certfront: Address<TAccountCertfront>;
};

export function getIncrementInstruction<
  TAccountCertfront extends string,
  TProgramAddress extends Address = typeof CERTFRONT_PROGRAM_ADDRESS,
>(
  input: IncrementInput<TAccountCertfront>,
  config?: { programAddress?: TProgramAddress }
): IncrementInstruction<TProgramAddress, TAccountCertfront> {
  // Program address.
  const programAddress = config?.programAddress ?? CERTFRONT_PROGRAM_ADDRESS;

  // Original accounts.
  const originalAccounts = {
    certfront: { value: input.certfront ?? null, isWritable: true },
  };
  ...
  const instruction = {
    accounts: [getAccountMeta(accounts.certfront)],
    programAddress,
    data: getIncrementInstructionDataEncoder().encode({}),
  } as IncrementInstruction<TProgramAddress, TAccountCertfront>;
